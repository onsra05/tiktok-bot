...
from models import Base, UserHistory, Product
...
# LOG CLICK
async def log_click(user_id, product_id):
...
async def deal(message: types.Message):
    ...
    products = await get_recommendations(user_id)

    if not products:
        await message.answer("Không có sản phẩm")
        return
